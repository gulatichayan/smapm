...
router.get('/', function(req, res, next) {
	sess = req.session;
	if(sess.email) {
		res.render('aboutUs', {firstName: sess.firstName, lastName: sess.lastName, email: sess.email, navbar: navBarLoggedIn});
	} else {
		res.render('aboutUs', {firstName: "Guest", lastName: "User", email: "Login to enjoy all features", navbar: navBarLoggedOut});
	}
});
