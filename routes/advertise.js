...
router.get('/', function(req, res, next) {
	sess = req.session;
	if(sess.email) {
		res.render('advertise', {firstName: sess.firstName, lastName: sess.lastName, email: sess.email, navbar: navBarLoggedIn});
	} else {
		res.render('advertise', {firstName: "Guest", lastName: "User", email: "Login to enjoy all features", navbar: navBarLoggedOut});
	}
});
